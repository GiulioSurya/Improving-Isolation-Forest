...
class DatasetPreparer:
    ...
    def _apply_perturbation(self, df: pd.DataFrame, idx: pd.Index):
        """
        Sostituisce, per ogni riga di idx, i valori degli indicatori con
        quelli della riga più distante (in senso euclideo) all’interno di idx.
        Robusta a:
          • un solo indicatore;
          • colonne lette come object / stringa;
          • dataset con NaN già rimossi a monte.
        """
        k = min(50, max(1, len(idx) // 4))  # Song et al.
        rng = np.random.default_rng(self.random_state)

        for i in idx:
            # ---------- riga corrente (shape --> (n_ind,)) -------------------
            y_ind_arr = (
                df.loc[i, self.ind_cols]
                .astype(float)  # forza numerico
                .to_numpy()
                .reshape(1, -1)  # shape (1, n_ind) anche se n_ind = 1
            )

            # ---------- candidati (escluso i) --------------------------------
            candidates_idx = rng.choice(idx.drop(i), size=min(k, len(idx) - 1),
                                        replace=False)
            candidates_arr = (
                df.loc[candidates_idx, self.ind_cols]
                .astype(float)  # forza numerico
                .to_numpy()  # shape (k, n_ind)
            )

            # ---------- distanza euclidea ------------------------------------

            dists = np.linalg.norm(candidates_arr - y_ind_arr, axis=1)

            # ---------- sostituzione con il più distante ---------------------
            y_prime = candidates_arr[np.argmax(dists)]
